...
plotSentiment <- function(df, title, subtitle) {
  ...
  plot <- nrc %>% ggplot(aes(x = Sentimiento, y = value, fill = source.name)) +
    ...
    coord_flip() + xlab("Sentimiento") + ylab("Puntaje") + labs(fill="Fuente") + scale_color_brewer()
...
getTopHeadLines <-
  function(obj) {
    ...
    {
      df_request <- df_request[grepl(cadena, 
                                      df_request$title, fixed = TRUE),] 
    } else {
      df_request <- df_request[grepl(cadena, 
                                     df_request$description, fixed = TRUE),]
    }
